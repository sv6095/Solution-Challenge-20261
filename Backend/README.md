...
- **The State Storage (`services.firestore_store`):** All structured data logic is persisted in Firestore collections. Context payloads are mirrored into tenant-scoped graph documents so risk and routing workflows can query customer network state without local disk state.
